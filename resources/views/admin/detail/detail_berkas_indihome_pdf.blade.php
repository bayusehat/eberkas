...
@php
    ...
    foreach ($lampiran as $l => $lv) {
        echo '<img src="'.asset('/lampiranfile/'.$lv->lampiran).'" alt="'.$lv->keterangan_lampiran.'" class="img-size"/><br>';
    }
@endphp
</div>
</div>
        ...
        html2canvas($(fbcanvas),
            {
                onrendered: function (canvas) {
                    ...
                    doc.deletePage(1);
                    doc.addPage(750, 1200);
                    doc.addImage(imgData, 'JPG', 0, 0,  750, 1200, undefined,'FAST');
                    doc.save(title);
                }
            });
    }
