        ...
        <hr>
            <div class="row">
                <div class="col-md-8 col-sm-12 col-xl-8">
                    <input type="text" class="form-control" name="nama_jenis_transaksi" id="nama_jenis_transaksi" placeholder="Nama Jenis Transaksi">
                    <small class="text-danger" id="valid_nama_jenis_transaksi"></small>
                </div>
...
<script>
    ...
    function loadData(){
        $('#tableData').DataTable({
            ...
            columns: [
                { name: 'id_jenis_transaksi', searchable: false, orderable: true, className: 'text-center' },
    ...
    function tambah(){
        var formData = $('#nama_jenis_transaksi').val();
        $.ajax({
            url : '{{ url("transaksi/jenis/insert") }}',
            headers : {
                'X-CSRF-TOKEN' : $('meta[name=csrf-token]').attr('content')
            },
            dataType : 'JSON',
            data : {
                'nama_jenis_transaksi' : formData
            },
            method : 'POST',
            success:function(res){
                if(res.status == 200){
                    $('#nama_jenis_transaksi').val('');
                    $('#nama_jenis_transaksi').focus();
    ...
    function show(id){
        btnToUpdate();
        $.ajax({
            url : '{{ url("transaksi/jenis/edit") }}/'+id,
            headers : {
                'X-CSRF-TOKEN' : $('meta[name=csrf-token]').attr('content')
            },
            dataType : 'JSON',
    ...
    function ubah(id){
        var formData = $('#nama_jenis_transaksi').val();
        $.ajax({
            url : '{{ url("transaksi/jenis/update") }}/'+id,
            headers : {
                'X-CSRF-TOKEN' : $('meta[name=csrf-token]').attr('content')
            },
            dataType : 'JSON',
    ...
    function deleteData(id){
        $.ajax({
            url : '{{ url("transaksi/jenis/delete") }}/'+id,
            headers : {
                'X-CSRF-TOKEN' : $('meta[name=csrf-token]').attr('content')
            },
            dataType : 'JSON',
